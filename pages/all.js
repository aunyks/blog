import LinksPage from 'components/LinksPage'
import Link from 'components/Link'

export default function ArchivePage() {
  return (
    <LinksPage
      pageTitle="Posts"
      title="Posts"
      subtitle="In reverse chronological order...pretty much">
      <Link href="/2021/11/you-wouldnt-download-a-guitar-right">
        you wouldn't download a guitar, right?
      </Link>
      <Link href="/2021/10/classifying-innovation">classifying innovation</Link>
      <Link href="/2021/7/procedural-skydomes-on-the-web">
        procedural skydomes on the web
      </Link>
      <Link href="/2021/6/in-search-of-the-gamedao">
        in search of the GameDAO
      </Link>
      <Link href="/2021/5/so-you-wanna-be-cutting-edge">
        so you wanna be cutting edge
      </Link>
      <Link href="/2021/4/making-a-dual-ps5-controller-stand" video>
        making a dual PS5 controller stand
      </Link>
      <Link href="/2021/2/3d-graphics-overview-high-level">
        3D graphics overview: high level
      </Link>
      <Link href="/2021/2/on-balance">on balance</Link>
      <Link href="/2021/1/gab-security-vulns">gab security vulns</Link>
      <Link href="/2020/12/toon-shading-in-blender">
        toon shading in blender
      </Link>
      <Link href="/2020/12/a-brief-lesson-on-gogo">a brief lesson on gogo</Link>
      <Link href="/2020/12/writing-for-expression-not-argument">
        writing for the sake of expression, not argument
      </Link>
      <Link href="/2020/11/demodulating-iq-data" series>
        demodulating i/q data
      </Link>
      <Link href="/2020/11/understanding-complex-signals" series>
        understanding complex signals
      </Link>
      <Link href="/2020/11/roam-the-web-human-hive-mind">
        roam, the web, and the human hive mind
      </Link>
      <Link href="/2020/10/why-i-care-about-radio">why i care about radio</Link>
      <Link href="/2020/10/last-minute-downlink">last minute downlink</Link>
      <Link href="/2020/10/making-serenitys-arrival">
        making serenity's arrival
      </Link>
      <Link href="/2020/10/music-is-a-language">music is a language</Link>
      <Link href="/2020/9/how-3d-design-works">how 3D design works</Link>
      <Link href="/2020/9/making-haze">making haze</Link>
      <Link href="/2020/9/instruments-on-the-web">instruments on the web</Link>
      <Link href="/2020/9/future-of-tech-for-we">
        the future of tech for we
      </Link>
      <Link href="/2020/9/hidden-in-plain-sight">hidden in plain sight</Link>
      <Link href="/2020/8/my-school-map-sucks">my school map sucks</Link>
      <Link href="/2020/8/making-the-orb">making the orb</Link>
      <Link href="/2020/8/designing-my-first-credit-card">
        designing my first credit card
      </Link>
      <Link href="/2020/8/intro-to-aleatoric-music">
        intro to aleatoric music
      </Link>
      <Link href="/2020/8/the-cycle-of-human-progress">
        the cycle of human progress
      </Link>
      {/*<Link href="/2020/8/to-be-american-is-to-rebel">to be american is to rebel</Link>*/}
      <Link href="/2020/8/to-be-a-jack-of-all-trades">
        to be a jack of all trades
      </Link>
      <Link href="/2020/7/3d-modeling-meets-fashion-consumers">
        3D modeling meets fashion consumers
      </Link>
      <Link href="/2020/6/mixed-reality-in-transit">
        mixed reality in transit
      </Link>
      <Link href="/2020/6/little-internets">little internets</Link>
      <Link href="/2020/4/techwear-cyberpunk-in-contemporary-fashion">
        techwear: cyberpunk in contemporary fashion
      </Link>
      <Link href="/2020/4/masks-will-be-the-norm">masks will be the norm</Link>
      <Link href="/2020/3/substack-open-redirect">
        substack open url redirection / reflected xss
      </Link>
      <Link href="/2020/3/replit-open-redirect">
        repl.it open url redirection
      </Link>
      <Link href="/2020/2/howd-i-get-here">how'd i get here?</Link>
      <Link href="/2019/12/key" series>
        key
      </Link>
      <Link href="/2019/11/musical-scales" series>
        musical scales
      </Link>
      <Link href="/2019/11/the-octave" series>
        the octave
      </Link>
      <Link href="/2019/11/how-hacking-works">how hacking works</Link>
      <Link href="/2019/11/learning-music-theory" series>
        learning music theory
      </Link>
    </LinksPage>
  )
}
