...
`}</CodeBlock>
      ...
      <CodeSnippet title="Delete Rows that Satisfy a Condition">
        <p>
          Deletes all rows in <code>my_table</code> that have a <code>column_b</code> value
...
`}</CodeBlock>
      ...
      <CodeSnippet title="Update Rows that Satisfy a Condition">
        <p>
          Updates all rows in <code>my_table</code> with <code>column_b</code> equal to <code>5</code>
          so that their <code>column_a</code> now equals <code>'no longer cool'</code> and <code>column_b</code> now equals <code>-3</code>.
        </p>
        <CodeBlock lang="sql">{`
UPDATE my_table
...
`}</CodeBlock>
      ...
      <CodeSnippet title="Get Rows that Satisfy a Condition">
        <p>
          Retrieves <code>column_a</code> and <code>column_b</code> of every row in <code>my_table</code> which
          has a <code>column_a</code> value equal to <code>'this is cool'</code>.
        </p>
        <CodeBlock lang="sql">{`
...
`}</CodeBlock>
      ...
      <CodeSnippet title="Get Certain Columns of Every Row">
        <p>
          Retrieves <code>column_a</code> and <code>column_b</code> of every row in <code>my_table</code>.
        </p>
        <CodeBlock lang="sql">{`
...
`}</CodeBlock>
      ...
      <CodeSnippet title="Insert a Row">
        <p>
          Adds data to <code>my_table</code>. This statement would still function with <code>column_a</code> and <code>'this is cool'</code> removed,
          since <code>column_a</code> can be <code>NULL</code> (see below).
        </p>
        <CodeBlock lang="sql">{`
INSERT my_table(
  column_a,
...
`}</CodeBlock>
      ...
      <CodeSnippet title="Create a Table">
        <p>
          Creates a new table called <code>my_table</code>, asuming it does not already
          exist. <code>column_a</code> can contain strings up to 500 characters in length or <code>NULL</code>.
          And, <code>column_b</code> can only contain integers.
        </p>
        <CodeBlock lang="sql">{`
